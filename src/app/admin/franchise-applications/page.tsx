...
function FranchiseApplicationsContent() {
  ...
  const handleGet = async () => {
    try {
      setLoading(true);
      const response = await axios.get("/api/franchise-apply");
        ...
        <table className="w-full border border-gray-300 text-sm">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="border px-3 py-2">Investment Capacity</th>
              ...
              return pageItems.length > 0 ? (
                pageItems.map((app, i) => (
                  <tr key={app._id || i}>
                    ...
                    <td className="border px-3 py-2">{app.businessName}</td>
                    <td className="border px-3 py-2">{app.location}</td>
                    <td className="border px-3 py-2">{app.yearsOfExperience} years</td>
                    <td className="border px-3 py-2">{app.investmentCapacity}</td>
                    <td className="border px-3 py-2 text-xs">{app.businessBackground?.substring(0, 30)}...</td>
...
export default function FranchiseApplicationsPage() {
  return (
    <AdminPageLayout title="Franchise Applications" section="franchise">
      <FranchiseApplicationsContent />
    </AdminPageLayout>
  );
}
